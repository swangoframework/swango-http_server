...
class Varchar extends \Swango\HttpServer\Validator {
    private $min_length, $max_length, $more_strict;
    public function __construct($cnkey, $min_length = 0, $max_length = 4096, ?bool $more_strict = false) {
    ...
    public function getCnMsg(): string {
        if (isset($this->cnmsg))
            return $this->cnmsg;
        $ret = $this->cnkey . '必须为字符串，长度为' . $this->min_length . '~' . $this->max_length . '，不得含有' . ($this->more_strict ? '( ) = \' ` % " \\ & < >' : '\\ & < >');
